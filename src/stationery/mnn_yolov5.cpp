...
#include "mnn_yolov5.h"


using mnncv::MNNYoloV5;

void nms(std::vector<types::Boxf> &input_boxes, float NMS_THRESH) {
    std::sort(input_boxes.begin(), input_boxes.end(), [](types::Boxf a, types::Boxf b) { return a.score > b.score; });
...
MNNYoloV5::MNNYoloV5(const std::string &_mnn_path, unsigned int _num_threads) 
{
  initialize_pretreat();

   mnn_interpreter = std::shared_ptr<MNN::Interpreter>(MNN::Interpreter::createFromFile(_mnn_path.c_str()));
...
void MNNYoloV5::initialize_pretreat()
{
...
inline void MNNYoloV5::transform(const cv::Mat &mat_rs)
{
  // normalize & HWC -> CHW & BGR -> RGB
  pretreat->convert(mat_rs.data, input_width, input_height, mat_rs.step[0], input_tensor);
}

void MNNYoloV5::resize_unscale(const cv::Mat &mat, cv::Mat &mat_rs,
...
void MNNYoloV5::detect(const cv::Mat &mat, std::vector<types::Boxf> &detected_boxes,
                       ...
                       unsigned int topk, unsigned int nms_type)
{
  ...
  int img_width = static_cast<int>(mat.cols);
   cv::Mat image;
    cv::resize(mat, image, cv::Size(input_width, input_height));

    YoloV5ScaleParams scale_params;
  float w_r = (float) input_width / (float) img_width;
  float h_r = (float) input_height / (float) img_height;
  //float r = std::min(w_r, h_r);
  scale_params.w_r = w_r;
  scale_params.h_r = h_r;
  scale_params.dh = 0;
  scale_params.dw = 0;
//    cv::Mat mat_rs;
...
//
    ...
    std::vector<int> dims{1, input_width, input_height, 3};
  ...
  nms(bbox_collection, 0.5);
detected_boxes = bbox_collection;
  //this->nms(bbox_collection, detected_boxes, iou_threshold, topk, nms_type);
}

void MNNYoloV5::generate_bboxes(const YoloV5ScaleParams &scale_params,
                                std::vector<types::Boxf> &bbox_collection,
                                ...
                                int img_width)
{
  ...
  float wr_ = scale_params.w_r;
  float hr_ = scale_params.h_r;
  int dw_ = scale_params.dw;
  int dh_ = scale_params.dh;

  //std::cout<<"r_" << r_<<std::endl;
  std::cout<<"dw_ "<< dw_ <<std::endl;
  std::cout<<"dh_ "<< dh_<<std::endl;
  ...
  for (unsigned int i = 0; i < num_anchors; ++i)
  {
    ...
    float x1 = ((cx - w / 2.f) - (float) dw_) / wr_;
    float y1 = ((cy - h / 2.f) - (float) dh_) / hr_;
    float x2 = ((cx + w / 2.f) - (float) dw_) / wr_;
    float y2 = ((cy + h / 2.f) - (float) dh_) / hr_;
...
// void MNNYoloV5::nms(std::vector<types::Boxf> &input, std::vector<types::Boxf> &output,
