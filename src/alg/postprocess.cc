...
int loadLabelName(const char* locationFilename, char* label[])
{
  ...
  //readLines(locationFilename, label, );
  return 0;
}
...
static int process(uint8_t* input, int* anchor, int grid_h, int grid_w, int height, int width, int stride,
                   ...
                   uint32_t zp, float scale, int obj_class_num)
{
  const int PROP_BOX_SIZE = obj_class_num + 5;
  int     validCount = 0;
...
int post_process(uint8_t* input0, uint8_t* input1, uint8_t* input2, int model_in_h, int model_in_w,
                 ...
                 std::vector<uint32_t>& qnt_zps, std::vector<float>& qnt_scales, detect_result_group_t* group , int obj_class_num)
{
  static int init = -1;
  // if (init == -1) {
  //   int ret = 0;
