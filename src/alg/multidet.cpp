...
int MultiDet::Init(const std::string path, float thresh) {
  //std::cout<<"11box_conf_threshold = "<< conf_thresh_<<std::endl;
  conf_thresh_ = thresh;
  return engine_.Init(path);
}

int MultiDet::Process(cv::Mat &img, std::vector<cv::Rect> &rects, std::vector<float> & scores, std::vector<int> & cls) {
  if(img.empty()) {
    return -1;
  }
  int width = 640;
  ...
  std::vector<float> out_scales = {0.127843, 0.113217, 0.103272};
  std::vector<uint32_t> out_zps = {187, 182, 172};
