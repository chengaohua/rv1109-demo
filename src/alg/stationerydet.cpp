...
#include "stationerydet.h"
#include "../tensor.h"
#include "postprocess.h"

#include "../common/alignface.h"

StationeryDet::StationeryDet() {}

StationeryDet::~StationeryDet() {

}

int StationeryDet::Init(const std::string path, float thresh) {
  //std::cout<<"11box_conf_threshold = "<< conf_thresh_<<std::endl;
  conf_thresh_ = thresh;
  return engine_.Init(path);
}

int StationeryDet::Process(cv::Mat &img, std::vector<cv::Rect> &rects, std::vector<float> & scores, std::vector<int> & cls) {
  if(img.empty()) {
    return -1;
  }
  int width = 640;
  ...
  std::vector<float> out_scales = {0.195779, 0.119053, 0.139349};
  std::vector<uint32_t> out_zps = {190, 146, 136};
